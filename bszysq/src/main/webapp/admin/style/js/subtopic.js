...
Z.prototype = {
	...
	topic:{
		init: function(){
			var zz = z.topic;
			zz.tid = m_i.ssub_val('jar.map.tid') || -1;
			zz.topic = m_i.ssub_val('jar.result') || {};
		}
	},
	// TODO 表格
	dg:{
...
//					}}
		...
		do_del: function(){
			...
			M.eu.dg_del(dg, 'id', 'name', function(ids, sels){
				...
				var ps = {ids: ids};
				$.ajax({
					url : "/admin/art/subtopic/dels.json",
					data : ps,
					success : function(d) {
		...
		do_dis: function(){
			...
			M.eu.dg_ids_opts(dg, 'id', 'name', '禁用', function(ids, sels){
				...
				$.ajax({
					url : "/admin/art/subtopic/diss.json",
					data : ps,
					success : function(d) {
						$.messager.progress('close');
						if(d.success){
	...
	add:{
		init: function(){
			...
			// 表单
			zz.frm = $('#addMgrForm');
			var tpd = z.topic.topic;
			zz.def_fvs = {pid: tpd.id, cid: tpd.cid, 
					id:'', name:'', img:'', sortn:'0', brief:'', isdel:'', imgFile:''};
		},
		/** 打开对话框 */
		dlg_open: function(){
			var zz = z.add;
		...
		submit: function(){
			...
			M.ajaxSubmit(zz.frm, {
				url: "/admin/art/subtopic/save.json",
				onSubmit: function(){
