...
public class SysmsgService extends BaseService<Sysmsg, SysmsgMapper> {
	...
	public void comment(Comment mo){
		...
		smsg.setTypex(Sysmsg.Typex_Art);
	...
	public void user(Comment mo){
		...
		smsg.setContent(mo.getContent());
		smsg.setUserid(mo.getTouserid());
		smsg.setTypex(Sysmsg.Typex_Sys);
		smsg.setExtra(String.valueOf(mo.getArtid()));
		commMsg(smsg);
	}
	...
	/** 发送推送(范围) */
	@Transactional
	public String commMsg(Sysmsg smsg){
		String emsg = null;
		...
		String gmsg = AjaxResultUtil.toJsonStr(gmo);
		try {
			GetuiUtil.pushMsgToApp(gmsg, smsg.getRangx());
			emsg = null;
		} catch (Exception e1) {
			emsg = "调用消息推送接口异常";
			e1.printStackTrace();
