...
/**
 * 文件 队列<br>
 * Base queue class, implements common functionality for a QueueFile-backed queue manager. This class is not thread safe; instances should be kept thread-confined.
 * <p>
 * The {@link #add(Object )}, {@link #peek()}, {@link #remove()}, and {@link #setListener(ObjectQueue.Listener)} methods may throw a {@link FileException} if the underlying {@link QueueFile}
 * experiences an {@link java.io.IOException}.
...
public class FileObjectQueue<T> implements ObjectQueue<T> {
	...
	private static class DirectByteArrayOutputStream extends ByteArrayOutputStream {
		...
		/**
		 * Gets a reference to the internal byte array. The {@link #size()} method indicates how many bytes contain actual data added since the last {@link #reset()} call.
		 */
